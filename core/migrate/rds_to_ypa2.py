...
def save_score(raw, recording):
    ...
    if raw.is_original:
        cooked.is_original = raw.is_original
    if raw.is_simple:
        cooked.is_simple =raw.is_simple
    cooked.play_type = raw.n_play_type_id
    cooked.asset_type = raw.n_asset_type_id
    cooked.key_play = raw.n_key_play
    cooked.status = 10
    # set cover_pic_id
    cooked.save()
