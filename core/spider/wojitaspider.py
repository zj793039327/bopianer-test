    ...
    def savescore2(self,url,title,artist):
        try:
            ...
            if pagecode:
                ...
                flag = 0
                extension = 'jpg'
                #if not os.path.exists(os.path.join('/home/jixin/Desktop/Resource/wojita/',id)):
                #    os.mkdir(os.path.join('/home/jixin/Desktop/Resource/wojita/',id))
                #for item in imgs:
                #    imgurl = 'http://www.wojita.com{0}'.format(item['src'])
                #    self.download(imgurl,'/home/jixin/Desktop/Resource/wojita/{0}/{1}'.format(id,flag))
                #    flag += 1
                #    extension = imgurl.split('/')[-1].split('.')[-1]
...
tqb = sooopu()
data = []
for page in range(1,663):
    url = 'http://www.wojita.com/jitapu/img/{0}.html'.format(str(page))
    pagecode = tqb.get(url)
    if pagecode:
        soup = BeautifulSoup(pagecode)
        try:
            link = soup.find('table',attrs={'class':'table'})
            pattern =re.compile('<tr>.*?[.\n]+.*?<td><a href="(.*?)">(.*?)</a></td>.*?[.\n]+.*?<td>.*?[.\n]+.*?<a.*?>(.*?)</a>')
            scorelist = re.findall(pattern,str(link))
            for item in scorelist:
                try:
                    if item[2] != '未知歌手':
                        link_url = 'http://www.wojita.com{0}'.format(item[0])
                        scorename = item[1]
                        artist = item[2]
                        score_info = tqb.savescore2(link_url,scorename,artist)
                        data.append(score_info)
                except:
                    pass
        except Exception,what:
            print what
    #with open('/home/jixin/Desktop/Resource/wojita/flag.txt','w') as flag:
    #    flag.write(str(page))
    #with open('/home/jixin/Desktop/Resource/wojita/scorelist.json','w') as f:
    #    json.dump(data,f)